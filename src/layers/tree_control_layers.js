...
// import "../custom-leaflet-classes/L.Control.Layers.Tree.ts";

// export const TreeControlLayer = () => {
//     const map = useMap();
//     const ctl = L.control.layers.tree(null, null,
//         {
//             namedToggle: true,
...
//     const hasAllUnSelected = function() {
...
import { useMap } from "react-leaflet";
// import '../custom-leaflet-classes/LeafletTreeLayerControl';
import L from 'leaflet';
require('leaflet.control.layers.tree');


export const TreeLayerControl = ({ baseTree, overlayTree, options }) => {
  ...
  useEffect(() => {
    ...
    // Check if L.control.layers.tree is available
    if (typeof L.control.layers.tree !== 'function') {
        console.error('L.control.layers.tree is not a function');
        return;
      }
  

    // Create the tree-like layer control using your extended library
    const treeLayerControl = L.control.layers.tree(baseTree, overlayTree, options);

    // Add the control to the Leaflet map
    treeLayerControl.addTo(map);
    // treeLayerControl.setOverlayTree(overlayTree);

    // Append the control container to your React component's DOM
    containerRef.current.appendChild(treeLayerControl.getContainer());

    // Ensure proper cleanup when the component unmounts
    return () => {
      treeLayerControl.remove();
    };
  }, [map, baseTree, overlayTree, options]);
  return <div className="leaflet-control-container">
        <div ref={containerRef} className="leaflet-top leaflet-right" /> 
    </div>
    

//   return <div ref={containerRef} className="tree-layer-control" />;
};
